...
export const CreateOrUpdateCombo = ({
  ...
  useEffect(() => {
    if (form.getValues("produtos").length >= 1) {
      setListProductSelecionados(combo?.produtos as ProdutosProps[]);
    }
  }, []);
  useEffect(() => {
  ...
  return isDesktop ? (
    ...
    >
      <DialogTrigger asChild>
        {combo ? (
          <Button>
            <Edit className="bg-transparent"></Edit>
          </Button>
        ) : (
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{combo ? "Atualizar Combo" : "Criar Combo"}</DialogTitle>
          <DialogDescription>
    ...
    >
      ...
      <DrawerContent className="p-4">
        <DrawerHeader>
          <DrawerTitle>{combo ? "Atualizar Combo" : "Criar Combo"}</DrawerTitle>
          <DrawerDescription>
            Preencha as informações do combo abaixo.
          </DrawerDescription>
        </DrawerHeader>
