...
export default function ComnbosDetalhes() {
  ...
  const handleDeleteProduto = async (produtoId: string) => {
    ...
    const updatedProdutos = combos.produtos.filter((produto) => produto.id !== produtoId);
      ...
      <article key={combos.id} className="border p-4 rounded ">
        <div className="flex justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button
              onClick={() => handleDeleteProduto(combos.id || "")}
              variant={"destructive"}
            >
              <Trash2 />
            </Button>
        ...
        <div className="flex items-center gap-4 justify-start text-sm text-gray-600">
          ...
          <div className="bg-gray-100 border rounded-md p-4">
            <h1 className="text-xl font-semibold border-b pb-4">Produtos do combo</h1>
                ...
                <AccordionItem key={item.id+"|"+index} value={"item"+index}>
                    <AccordionTrigger>
                        <div className="flex justify-between items-center p-4 ">
                            ...
                            <span className="text-gray-500">R$ {item.valor.toFixed(2)}</span>
                        </div>
                    </AccordionTrigger>
                    <AccordionContent className="border-t">
                        <div className="p-4">
                            ...
                            <p><strong>Criado em:</strong> {item.created_at}</p>
                        </div>
                        <div className="flex justify-end">
                        <Button variant={"destructive"} onClick={()=>handleDeleteProduto(item.id as string)}><Trash2></Trash2> Deletar</Button></div>
                    </AccordionContent>
                </AccordionItem>
            ))}
        </Accordion>
