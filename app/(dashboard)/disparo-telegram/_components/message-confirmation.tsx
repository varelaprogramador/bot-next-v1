...
import { Send, Loader2, CheckCircle2, AlertCircle, XCircle } from "lucide-react";
...
export default function MessageConfirmation({
    ...
    const sendMessages = async () => {
        ...
        try {
            ...
            if (telegramContacts.length < 10) {
                let successCount = 0;
                let errorCount = 0;

                for (let i = 0; i < telegramContacts.length; i++) {
                    const userId = telegramContacts[i];
                    const currentProgress = Math.floor((i / telegramContacts.length) * 100);
            ...
            // For larger batches, use the batch API
                ...
                if (response.ok) {
                    ...
                    setResults({
                        successful: 0,
                        failed: telegramContacts.length,
                        total: telegramContacts.length,
                        status: "error",
                    });
                    ...
                    <Card className="border-none shadow-none">
                        <CardHeader className="p-0">
                            <h3 className="text-sm font-medium">
                                Prévia da mensagem para {telegramContacts.length} destinatário(s)
                            </h3>
                        </CardHeader>
                        ...
                        <CardContent className="flex justify-end pt-4 px-0">
                                ...
                                <p className="text-sm text-center text-muted-foreground">
                                    {progress < 100
                                        ? `Processando ${Math.floor(
                                            (telegramContacts.length * progress) / 100
                                        )} de ${telegramContacts.length} mensagens`
    ...
    function getButtonGridClass() {
        if (buttons.length === 1) return "grid-cols-1";
        if (buttons.length === 2) return "grid-cols-2";
        if (buttons.length === 3) return "grid-cols-1";
        if (buttons.length === 4) return "grid-cols-2";
        return "grid-cols-1";
    }
