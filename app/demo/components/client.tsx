...
interface ClientProps {
    handleUpdatePositions: (data: ProdutosLojaProps[]) => void;
}
            ...
            >
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 flex-1">
                    <SortableContext items={data} strategy={rectSortingStrategy} disabled={loading}>
                        ...
                        <DragOverlay>
                            {activeId ? (
                                <ProductCard product={data.find(i => i.id === activeId)!} />
                            ) : null}
                        </DragOverlay>
                    </SortableContext>
                </div>
...
const ProductCard = ({ product }: { product: ProdutosLojaProps }) => {
    ...
    return (
        <Card
            ...
            {...attributes}
            ref={setNodeRef}
            className={cn("overflow-hidden bg-gray-100 border shadow-none", isDragging && "opacity-50")}
            style={{
        ...
        >
            <CardContent className="p-4 max-md:flex gap-4 max-md:p-0">
                ...
                <div className="flex flex-col justify-center">
                    ...
                    <div className="text-green-600">no pix R${(product.valor || 0).toFixed(2)}</div>
                    <div className="text-gray-600">R${(product.valor * 1.1 || 0).toFixed(2)} no cartão</div>
                    <Button className="w-full bg-gray-900 hover:bg-gray-800 hidden max-md:flex">Comprar</Button>
                </div>
            </CardContent>
            <CardFooter>
                <Button className="w-full bg-gray-900 hover:bg-gray-800 flex max-md:hidden">Comprar</Button>
            </CardFooter>
        </Card>
    );
};